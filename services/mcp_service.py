...
@tool
async def get_pois(keywords: str, city: str) -> POISearchOutput:
...
@tool
async def get_location(address: str) -> LocationOutput:
...
@tool
async def get_distance(origins: str, destination: str) -> DistanceOutput:
    """测量两个经纬度坐标之间的距离并返回结果。
    ...
    Returns:
        dict: 距离结果，格式为：
            {
                "distance": int,   # 距离，单位为米
                "duration": int    # 预计耗时，单位为秒
            }
    """
    ...
    return DistanceOutput(
        distance=data.get("results", [{}])[0].get("distance", ""),
        duration=data.get("results", [{}])[0].get("duration", ""),
    ).model_dump()


@tool
async def tavily_search(query: str, max_results: int = 5) -> TavilySearchOutput:
